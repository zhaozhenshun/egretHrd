class BlockHorizontal extends EntityBlock implements Enity{
	public limitMove = {
	left:0,
	right:256
	};
	...
	public constructor() {
		super();
		this.initEvt();
		this.limitMove.right = 256;
	}
	private block:egret.Bitmap;
	private initEvt(){
		
		let block = Resouce.createBitmapByName('hrd_json.blok-h');
		this.block = block;
	...
	public onMouseDownFun(){
		if (!this.tweenActive) {
                ...
                // this.movementPosX = this.pointer.pos.x;
                 ...
                 for (var c = this.x, b = this.y, d = 0, f = 0; 64 <= c;) d++,
                    c -= 64;
                for (; 64 <= b;) f++, b -= 64;
                ...
                for (d += 2; 6 > d;) {
                    ...
                    if (1 == b) {
                        this.limitMove.right = 64 * (d - 2);
                        break
                    }
                    d++
                }
            }
			console.log("limitMove", this.limitMove);
	}
